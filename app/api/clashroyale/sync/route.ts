// app/api/clashroyale/sync/route.ts — the admin's manual "Sync now" trigger.
// Actual sync logic lives in lib/clashRoyaleSync.ts, shared with the
// automatic background refresh (see syncClashRoyaleIfStale there).
//
...
import { syncClashRoyale } from "@/lib/clashRoyaleSync";
...
export async function POST() {
  ...
  const result = await syncClashRoyale();
